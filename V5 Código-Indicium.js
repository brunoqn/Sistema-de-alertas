...
function executarAlerta() {
  ...
  for (let i = 1; i < dados.length; i++) {
    ...
    // Lógica atualizada para usar a data de conclusão da Coluna Q
    } else if (datasIguais(dataConclusao, hojeString, fusoHorario) && !statusAtual.includes("sent_conclusion_feedback")) {
      enviarEmail(emailAfilhado, "Avalie sua Experiência no Programa de Apadrinhamento", criarMensagemFeedback90Dias(nomeAfilhado));
      planilha.getRange(i + 1, COL_STATUS + 1).setValue(statusAtual + "sent_conclusion_feedback; ");
      Logger.log('>> E-mail de avaliação de conclusão enviado para ' + nomeAfilhado);
    }
  }
