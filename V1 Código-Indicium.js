function executarAlerta() {
  ...
  Logger.log("--- Iniciando execução com correção final de Charset ---");
  ...
  for (let i = 1; i < dados.length; i++) {
    ...
    if (statusAtual.toString().includes(hojeString) || !linha[COL_EMAIL_AFILHADO]) {
      continue;
    }
    ...
    if (datasIguais(linha[COL_38_DIAS], hojeString, fusoHorario)) {
    ...
    if (datasIguais(linha[COL_75_DIAS], hojeString, fusoHorario)) {
      const assunto = `📌 Alerta – 75 dias de experiência: ${nomeAfilhado}`;
      const corpoHtml = criarMensagem75Dias(nomeAfilhado);
      enviarEmail(seuEmail, assunto, corpoHtml, opcoesBcc);
      emailEnviado = true;
    }
    if (datasIguais(linha[COL_83_DIAS], hojeString, fusoHorario)) {
...
// *** FUNÇÃO FINAL E CORRIGIDA para enviar e-mails ***
function enviarEmail(destinatario, assunto, corpoHtml, opcoes) {
  ...
  // A CORREÇÃO CRÍTICA E FINAL: Força o conjunto de caracteres para o corpo do e-mail.
